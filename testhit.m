% Running with below to test A detector
% weights = [-2, -2, 2, 2, -2, 2, 2, -2, 2, -2, 2, -2, -2, 2, 2, -2  -2, -2, 2, 2]
% bias = 1.5
% Running with below to test E detector
% weights = [6, 10, -2, -2,  2,  2,  2, -2, -6,  2, 10, -2,  6,  2,  2, -2, 10, -10, -10, -2]
% bias = -14.5
%Syntax: hit = testhit(weights, bias);
%====####===change the tuple number to test each TSET

function hit = testhit(w, b);
...
[~,cpats] = size(PATS2);
    for patnum = 1: cpats
        close all;
        a = hardlims(w*PATS2(:,patnum)+b)
        flag = patnum
        figure; chkbrd1 ( ctom(PATS2(:,patnum),4,5));
        pause;
    end
